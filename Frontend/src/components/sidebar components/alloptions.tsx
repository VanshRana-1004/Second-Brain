...
export function AllOptions( props : AllOptionsProps){
    ...
    const fxnArr=[()=>{Display("home")},()=>{props.setIcon(!props.icon)},()=>{Display("twitter")},()=>{Display("youtube")},()=>{Display("website")},()=>{Display("notion")},()=>{Display("instagram")}];

    const style=`overflow-hidden flex flex-col py-3 px-3 gap-3 h-96 ${props.showSideBar?'justify-start':'justify-center' }  `
   
    return <div className={style}>
        {Array(7).fill(1).map((x,index)=><Option key={index+x} text={optionArr[index]} Logo={iconsArr[index]} onClick={fxnArr[index]} hover={hoverArr[index][0]} setHover={hoverArr[index][1]} showSideBar={props.showSideBar} icon={props.icon} />)}
    </div>
}
